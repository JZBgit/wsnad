...
if __name__ == '__main__':
    ...
    # 24 21 19 18 12 8    21  18      20 0  阈值10    17 1

    node=20
    # plt.figure()
    # plt.plot(win_tem[node][0])
    plt.figure()
    plt.plot(da_tem[node][0],label='input')
    # plt.figure()
    plt.plot(data_tem[node][0],"--",label='reconstruction')
    plt.legend(loc=4,fontsize=30)
    plt.xlabel('timestamp', fontdict={'family' : 'Times New Roman', 'size':45},labelpad=5)#labelpad为标题距离刻度线范围
    plt.ylabel('value', fontdict={'family' : 'Times New Roman', 'size':45},labelpad=5)
    plt.xticks(fontproperties = 'Times New Roman', size = 40)
    plt.yticks(fontproperties = 'Times New Roman', size = 40)
    # plt.tight_layout()
    plt.subplots_adjust(left=0.07, right=0.99, bottom=0.1, top=0.99)

    plt.figure()
    err=np.square(data_tem[node][0]-da_tem[node][0])
    plt.plot(err,label='score')
    plt.axhline(y=5,color='r',label='threshold')
    ...
    plt.yticks(fontproperties = 'Times New Roman', size = 40)
    # plt.tight_layout()
    plt.subplots_adjust(left=0.06, right=0.99, bottom=0.1, top=0.99)

    plt.figure()
    plt.plot(label_tem[node][0],label='anomaly label')
    plt.legend(loc=1,fontsize=30)
    plt.xlabel('timestamp', fontdict={'family' : 'Times New Roman', 'size':45},labelpad=5)#labelpad为标题距离刻度线范围
    plt.ylabel('value', fontdict={'family' : 'Times New Roman', 'size':45},labelpad=5)
    plt.xticks(fontproperties = 'Times New Roman', size = 40)
    ...
    plt.figure()
    plt.plot(np.where(err>=5,1,0),color='#ff7f0e',label='anomaly detection')
    plt.legend(loc=1,fontsize=30)
    plt.xlabel('timestamp', fontdict={'family' : 'Times New Roman', 'size':45},labelpad=5)#labelpad为标题距离刻度线范围
    plt.ylabel('value', fontdict={'family' : 'Times New Roman', 'size':45},labelpad=5)
    plt.xticks(fontproperties = 'Times New Roman', size = 40)
    ...
    # #     plt.plot(da_tem[n].T)
    # plt.figure()
    # plt.plot(win_tem[node].T)
    # plt.figure()
    # plt.plot(data_tem[node].T)
    # plt.figure()
    # plt.plot(da_tem[node].T)
    # # # plt.figure()
    # # # plt.plot(cha[0].T)
    # plt.figure()
    # plt.plot(label_tem[node].T)
    # plt.show()
